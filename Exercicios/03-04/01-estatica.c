...
bool s_empty(Pilha *pilha) {
    if(pilha == nullptr) return false;

    return pilha->topo == -1;
}

bool s_push(Pilha *pilha, char valor) {
    if(pilha == nullptr) return false;
...
void s_pop(Pilha *pilha) {
    if(pilha == nullptr) return;

    pilha->topo--;
}

char s_top(Pilha *pilha) {
    if(pilha == nullptr) return -1;
    if(pilha->topo >= 255) return -1;
